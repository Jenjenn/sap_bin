...
while(<>) {
  ...
  #ignore the interrupt vector
  elsif (m/(LinStackBacktrace)|(LinStack)|(CTrcStack2)|(ThStackHandler)|(DpTrcOnOff)|(libpthread\.so\.0\[T\])/){
  
  }
  #line is a part of the stack
  elsif(m/\[[A-Z]\]\((.*?)\+0x[a-f0-9]*\)\[0x[a-f0-9]*/){
  #elsif(m/\[[A-Z]\]\((.*?)\+0x[a-f0-9]*\)\[0x[a-f0-9]*,0x[a-f0-9]*\]/){
	my $function = $1;
	if ($current eq "") {
      $current = $function;
    } else {
  ...
  #line is not a part of the stack
  elsif(!($current eq "")) {
    $stacks{$current} += 1;
    $current = "";
  }
